...
public:
  ...
  void setTicks(const std::vector<double> &positions, const std::vector<QString> labels);
  ...
  void addTicks(const std::vector<double> &positions, const std::vector<QString> &labels);
...
public:
  ...
  // getters:
  size_t size() const { return mData.size()-mPreallocSize; }
  ...
  void set(const std::vector<DataType> &data, bool alreadySorted=false);
  void add(const QCPDataContainer<DataType> &data);
  void add(const std::vector<DataType> &data, bool alreadySorted=false);
  ...
  const_iterator constBegin() const { return mData.cbegin()+mPreallocSize; }
  const_iterator constEnd() const { return mData.cend(); }
  ...
  QCPDataRange dataRange() const { return QCPDataRange(0, (int)size()); }
...
*/
template <class DataType>
void QCPDataContainer<DataType>::set(const std::vector<DataType> &data, bool alreadySorted)
{
  mData = data;
  mPreallocSize = 0;
  mPreallocIteration = 0;
...
*/
template <class DataType>
void QCPDataContainer<DataType>::add(const std::vector<DataType> &data, bool alreadySorted)
{
  if (data.empty())
    return;
  if (isEmpty())
  ...
  const size_t n = (int)data.size();
  const int oldSize = (int)size();
  
  if (alreadySorted && oldSize > 0 && !qcpLessThanSortKey<DataType>(*constBegin(), *(data.cend()-1))) // prepend if new data is sorted and keys are all smaller than or equal to existing ones
  {
    if (mPreallocSize < n)
      preallocateGrow(n);
    mPreallocSize -= n;
  ...
  {
    mData.resize(mData.size()+n);
    std::copy(data.cbegin(), data.cend(), end()-n);
...
{
  if (minimumPreallocSize <= (int)mPreallocSize)
    return;
  ...
  int sizeDifference = newPreallocSize - (int)mPreallocSize;
...
{
  if (index >= 0 && index < (int)mDataContainer->size())
  {
...
double QCPAbstractPlottable1D<DataType>::selectTest(const QPointF &pos, bool onlySelectable, QVariant *details) const
{
  ...
  int minDistIndex = (int)mDataContainer->size();
  ...
  if (minDistIndex != (int)mDataContainer->size())
...
public:
  ...
  */
  ...
  void setData(const std::vector<double> &t, const std::vector<double> &keys, const std::vector<double> &values, bool alreadySorted=false);
  ...
  // non-property methods:
  void addData(const std::vector<double> &t, const std::vector<double> &keys, const std::vector<double> &values, bool alreadySorted=false);
...
public:
  ...
  QCPStatisticalBoxData(double key, double minimum, double lowerQuartile, double median, double upperQuartile, double maximum, const std::vector<double>& outliers=std::vector<double>());
  ...
  inline QCPRange valueRange() const
  {
    ...
    for (std::vector<double>::const_iterator it = outliers.cbegin(); it != outliers.cend(); ++it)
      result.expand(*it);
    return result;
  }
...
public:
  ...
  void setData(const std::vector<double> &keys, const std::vector<double> &minimum, const std::vector<double> &lowerQuartile, const std::vector<double> &median, const std::vector<double> &upperQuartile, const std::vector<double> &maximum, bool alreadySorted=false);
  void setWidth(double width);
  ...
  // non-property methods:
  void addData(const std::vector<double> &keys,
               const std::vector<double> &minimum,\
               const std::vector<double> &lowerQuartile,
               const std::vector<double> &median,
               const std::vector<double> &upperQuartile,
               const std::vector<double> &maximum,
               bool alreadySorted=false);
  void addData(double key, double minimum, double lowerQuartile, double median, double upperQuartile, double maximum, const std::vector<double> &outliers=std::vector<double>());
...
//
//   inline double sortKey() const { return key; }
//   inline static Candle fromSortKey(double sortKey) { return Candle(sortKey, 0, 0, 0, 0, 0, 0); }
//   inline static bool sortKeyIsMainKey() { return true; }
//
//   inline double mainKey() const { return key; }
//   inline double mainValue() const { return close; }
//
...
//   VolumeType volume, interest;
// };
// Q_DECLARE_TYPEINFO(Candle, Q_PRIMITIVE_TYPE);
//
...
*/
typedef QCPDataContainer<Candle> QCPFinancialDataContainer;

class QCP_LIB_DECL QCPFinancial : public QCPAbstractPlottable1D<Candle>
{
  Q_OBJECT
  /// \cond INCLUDE_QPROPERTIES
...
public:
  ...
  */
  ...
  void setData(const std::vector<double> &keys, const std::vector<double> &open, const std::vector<double> &high, const std::vector<double> &low, const std::vector<double> &close, const std::vector<double> &volume, const std::vector<double> &interest);
  ...
  // non-property methods:
  void addData(const std::vector<double> &keys, const std::vector<double> &open, const std::vector<double> &high, const std::vector<double> &low, const std::vector<double> &close, const std::vector<double>& volume, const std::vector<double>& interest, bool alreadySorted=false);
  void addData(double key, double open, double high, double low, double close, double volume, double interest);
  ...
  // static methods:
  static QCPFinancialDataContainer timeSeriesToOhlc(const std::vector<double> &time, const std::vector<double> &value, double timeBinSize, double timeBinOffset = 0);
...
public:
  ...
  */
  ...
  void setData(const std::vector<double> &error);
  void setData(const std::vector<double> &errorMinus, const std::vector<double> &errorPlus);
  ...
  // non-property methods:
  void addData(const std::vector<double> &error);
  void addData(const std::vector<double> &errorMinus, const std::vector<double> &errorPlus);
