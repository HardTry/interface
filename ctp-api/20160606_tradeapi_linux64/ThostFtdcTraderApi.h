...
public:
  ...
  ///        0x2002 发送心跳失败
  ///        0x2003 收到错误报文
  virtual void OnFrontDisconnected(int nReason) { UNUSED(nReason); }

  ///心跳超时警告。当长时间未收到报文时，该方法被调用。
  ///@param nTimeLapse 距离上次接收报文的时间
  virtual void OnHeartBeatWarning(int nTimeLapse) { UNUSED(nTimeLapse); }
  ...
  ///报单通知
  virtual void OnRtnOrder(CThostFtdcOrderField *pOrder) { UNUSED(pOrder); }

  ///成交通知
  virtual void OnRtnTrade(CThostFtdcTradeField *pTrade) { UNUSED(pTrade); }
  ...
  ///报价通知
  virtual void OnRtnQuote(CThostFtdcQuoteField *pQuote) { UNUSED(pQuote); }
